local B2SpatialConvolution, parent = torch.class('nn.B2SpatialConvolution', 'nn.Module')

function B2SpatialConvolution:__init(nInputPlane, nOutputPlane, kW, kH, dW, dH)
   parent.__init(self)
...
function B2SpatialConvolution:reset(stdv)
   if stdv then
...
function B2SpatialConvolution:updateOutput(input)
...
function B2SpatialConvolution:updateGradInput(input, gradOutput)
   if input:nDimension() == 3 then
...
function B2SpatialConvolution:accGradParameters(input, gradOutput, scale)
   scale = scale or 1
   if input:nDimension() == 3 then
      gradOutput = input.new(gradOutput:storage(), 1, gradOutput:size(1), -1, gradOutput:size(2)*gradOutput:size(3), -1)

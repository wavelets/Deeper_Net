...
function loadParams(model,saved_model)
    ...
    local saved_params = saved_model:parameters(); 
      ...
      local bn= model:findModules("cudnn.SpatialBatchNormalization")
      local saved_bn= saved_model:findModules("cudnn.SpatialBatchNormalization")
      for i=1,#bn do
         bn[i].running_mean:copy(saved_bn[i].running_mean)
         bn[i].running_std:copy(saved_bn[i].running_std)
      end
end
...
--     local s = convNodes[i].weight:size()
--     local m = (convNodes[i].weight:norm(1))/n;
--     convNodes[i].weight:sign():mul(m)
--    end
--    if opt.nGPU >1 then
--     model:syncParameters()
--    end
